...
animate(bedtimemovie,
  [Scene(bedtimemovie, frame, 1:150, easingfunction=easeoutquad)],
  creategif=true, framerate=30,
  pathname="images/bezier/bedtimemovie.gif")
{% endhighlight julia %}
...
# we have to have a drawing open, otherwise
# textpath() doesn't know where to turn
...
# that's v0.6 syntax...
# v0.7 is df[[occursin(r".*Kabel.*", fname) for fname  in df[:Name]], :]
